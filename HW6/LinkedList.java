...
public class LinkedList<E> implements List<E>,Queue<E>{
    ...
    public Iterator<E> iterator() {
        Iterator<E> myOwnIter= new Iterator<>( (LinkedList<E>)this, obj);
        return myOwnIter;
    }
    ...
    @Override
    public boolean offer(E e) { //Adds e to end the queue.
        try{
            add(e);
        }
        catch(NullPointerException error){  //If null(empty)
            System.out.println("Error occured when offering "+ error.getMessage());
            return false;
        }
        return true;
    }
    ...
    @Override
    public E poll(){        //Will return and delete the head of this LinkedList.
        E return_value_will_delete;
        try{
            if( size() == 0 ) 
                remove( obj[0] );   //Which will cause error and it will catch and will return null.
            return_value_will_delete = obj[0];
            remove( obj[0] );
        }
        catch(IndexOutOfBoundsException error){
            System.out.println("Error about size! " + error.getMessage() );
            return null;
        }
        return return_value_will_delete;

    }

    @Override
    public E element() {    //Will return the head of this LinkedList.
        if( size() == 0 )
            throw new IndexOutOfBoundsException();
        E element_to_return;
        try{
            element_to_return = obj[0];
            return element_to_return;
        }
        catch(NoSuchElementException error){  //If null(empty)
            System.out.println("Error occured when offering "+ error.getMessage());
        }
        return null;
    }
