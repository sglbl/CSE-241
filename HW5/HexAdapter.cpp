...
template <template<class...> class T>
HexAdapter<T>::HexAdapter(int is_done): AbstractHex(is_done)
{ setSize(10); }

template <template<class...> class T>
HexAdapter<T>::HexAdapter(int &is_done, const string &loadfile): AbstractHex(is_done,loadfile)
{/*BODY INTENTIONALLY EMPTY*/}

template <template<class...> class T>
void HexAdapter<T>::setSize(const int& bs){
	boardSize = bs;
	reset();
}

template <template<class...> class T>
void HexAdapter<T>::createEmptyTable(int size){		//Creates an empty table.
	...
	for(int row=0; row<boardSize; row++){
		...
		for(int column=0; column<boardSize; column++)
			hexCells[row].push_back( HexAdapter::Cell(column, row, '.') );
	}
}

template <template<class...> class T>
void HexAdapter<T>::reset(){	        //This function is resets size.	
	deleteTable();
...
template <template<class...> class T>
void HexAdapter<T>::deleteTable(){
	for(int i=0; i< hexCells.size(); i++)
			hexCells.pop_back();	//Delete everything.
}

template <template<class...> class T>
bool HexAdapter<T>::playGame(int is_done){
...
template <template<class...> class T>
typename HexAdapter<T>::Cell HexAdapter<T>::play(){	//Play of computer
	Cell temp;
...
template <template<class...> class T>
bool HexAdapter<T>::play(Cell &c){		//Play of user
...
template <template<class...> class T>
int HexAdapter<T>::commandInput(string &coord, string filename){
	ofstream file;
	ifstream file2;
	bool askflag=false;
	do{
...
template <template<class...> class T>
int HexAdapter<T>::isEnd(int &flag){
...
template <template<class...> class T>
void HexAdapter<T>::makeLettersCapital(char check, int visitedplaces[26][26]){
	if(check=='x')
...
template <template<class...> class T>
bool HexAdapter<T>::is_valid_move(int x, int y, char letter){
...
template <template<class...> class T>
bool HexAdapter<T>::input_wrong(string coord){
...
template <template<class...> class T>
int HexAdapter<T>::size_of_string(const string &str){
	auto i=0;
...
template <template<class...> class T>
void HexAdapter<T>::setOldMove(const HexAdapter::Cell &c){			//Entering oldMove for undo operation.
...
template <template<class...> class T>
int HexAdapter<T>::check_full(int x, int y, int visitedplaces[][26], char check, int &score){
...
template <template<class...> class T>
void HexAdapter<T>::readFromFile(const string &filename){	//For reading the file.
	...
	reset();		//Resets everything in board to '.'

	for(i=0; i<boardSize; i++){
		file >> inside;
		for(j=0; j<boardSize; j++){
			if(inside[j] == '.' || inside[j] == 'x' || inside[j] == 'o')
				try{ hexCells[i][j].setStatus(inside[j]); }
				catch(const Errors& myerror){
...
template <template<class...> class T>
void HexAdapter<T>::writeToFile(const string &filename){		//Wrtiting to file.
	/*
...
template <template<class...> class T>
int HexAdapter<T>::getIsDone() const{
	return is_done2;
}

template <template<class...> class T>
typename HexAdapter<T>::Cell HexAdapter<T>::operator()(int x,int y){	//I used this operator() in operator== to compare.
	bool flag=true;
