...
class HexAdapter: public AbstractHex{	//Derived class
	public:
		HexAdapter();
		HexAdapter(int is_done);
		HexAdapter(int &is_done, const std::string &loadfile);
		//Constructors
	...
	private:
		T<T<Cell>> hexCells;
		//Template template (variadic) for holding hex cells.
