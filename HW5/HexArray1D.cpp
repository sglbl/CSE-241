...
HexArray1D::HexArray1D(int is_done): AbstractHex(is_done)
{  setSize(10); }

HexArray1D::HexArray1D(int &is_done, const string &loadfile): AbstractHex(is_done,loadfile)
{/*BODY INTENTIONALLY EMPTY*/}
...
HexArray1D::HexArray1D(const HexArray1D &HObject){ 		//Copy Constructor
	boardSize = HObject.boardSize;
	is_done2 = HObject.is_done2;
	how_many_player = HObject.how_many_player;
	turn = HObject.turn;
...
void HexArray1D::setSize(const int& bs){
	boardSize = bs;
	reset();
}

void HexArray1D::createEmptyTable(int size){		//Creates an empty table.
	for(int i=0; i<26*26; i++){	//Because there are 26 lettters so max 26*26 board.
		oldMoves[i].setStatus('.');
	}

	hexCells = (Cell*)calloc(size*size, sizeof(Cell));  //1D C Array for hexCells
    for(int i=0; i<size*size; i++)
        (hexCells[i]).setStatus('.');
    // currentPointer= (int*)realloc(currentPointer, 1000*counter*sizeof(int));
}

void HexArray1D::reset(){	        //This function is resets size.	
	deleteTable();
	createEmptyTable(boardSize);
}
...
bool HexArray1D::playGame(int is_done){
	...
	while ( isEnd(flag) == false && (getIsDone()==0 || getIsDone()==1) ){
		...
		if(how_many_player==1){				// 1 player game. 
			...
			hexCells[temp1*boardSize + temp0].setY(temp1);		//y value of cell.
			hexCells[temp1*boardSize + temp0].setX(temp0);		//x value of cell.

			if(play(hexCells[temp1*boardSize + temp0])==false)
				continue;
			//CHEKING IF USER WON
			...
			if(play(hexCells[temp1*boardSize + temp0])==false)	//Playing user.
				continue;
			//CHEKING IF USER WON
			else if(flag != 0)	//if not 0 then somebody won.
				break;
...
HexArray1D::Cell HexArray1D::play(){	//Play of computer
	Cell temp;
	...
	}while(hexCells[(randnum-1)*boardSize + randnum2-1].getStatus() != '.' || hexCells[(randnum-1)*boardSize + randnum2-1].getStatus() == 'x' );

	cout << "Choice of AI is " << char (randnum2-1 + 'A') << randnum << endl;
	
	hexCells[(randnum-1)*boardSize + randnum2-1].setStatus('o');			//Set status to 'o'.
	hexCells[(randnum-1)*boardSize + randnum2-1].setY(randnum -1);		                //y value of cell.
	hexCells[(randnum-1)*boardSize + randnum2-1].setX(randnum2-1);	                	//x value of cell.
	setOldMove( hexCells[(randnum-1)*boardSize + randnum2-1] );			                //Put this value for Undo.
	return hexCells[(randnum-1)*boardSize + randnum2-1];
}

bool HexArray1D::play(Cell &c){		//Play of user
...
int HexArray1D::commandInput(string &coord, string filename){
	ofstream file;
	ifstream file2;
	bool askflag=false;
	do{
		...
		if(coord.compare(0,5, "SAVE ",0, 5) == 0){	    //It compares "SAVE " with coord from 0th index to the 5 characters.
			...
			if(file2.is_open() == false)
				{ throw( Errors("Couldn't open file.\n") ); }
			else
	...
	for(int m=0; m<boardSize; m++){
		...
		for(int i=0; i<boardSize; i++)
    		for(int j=0; j<boardSize; j++)
    			visitedplaces[i][j]=0;
		if(hexCells[m*boardSize + 0].getStatus() == 'x' || hexCells[m*boardSize + 0].getStatus() == 'X')
		...
		if(hexCells[0*boardSize + m].getStatus() == 'o' || hexCells[0*boardSize + m].getStatus() == 'O')
...
void HexArray1D::makeLettersCapital(char check, int visitedplaces[26][26]){
	if(check=='x')
		for(int y = 0; y < boardSize; ++y)
			for(int x = 0; x < boardSize; ++x)
				if( visitedplaces[y][x]==1 ){
					hexCells[y*boardSize + x].setStatus('X');
					if(x==boardSize-2 && hexCells[y*boardSize + boardSize-1].getStatus() == 'x')		
						hexCells[y*boardSize + boardSize-1].setStatus('X');
					if(y>=1)
						if(x==boardSize-2 && hexCells[(y-1)*boardSize + boardSize-1].getStatus() == 'x')
							hexCells[(y-1)*boardSize + boardSize-1].setStatus('X');
				}
	if(check=='o')
		for(int y = 0; y < boardSize; ++y)
			for(int x = 0; x < boardSize; ++x)
				if( visitedplaces[y][x]==1 ){
					hexCells[y*boardSize + x].setStatus('O');
					if(y==boardSize-2 && hexCells[(boardSize-1)*boardSize + x].getStatus() == 'o')		
						hexCells[(boardSize-1)*boardSize + x].setStatus('O'); 
					if(x>=1)
						if(y==boardSize-2 && hexCells[(boardSize-1)*boardSize + x-1].getStatus() =='o')
								hexCells[(boardSize-1)*boardSize + x-1].setStatus('O');
				}
}

bool HexArray1D::is_valid_move(int x, int y, char letter){
	if( x>=0 && y>=0 && x<boardSize && y<boardSize && (hexCells[y*boardSize + x].getStatus()== letter
		 || hexCells[y*boardSize + x].getStatus()== letter-32 ) )	
		return true;

	return false;
}

bool HexArray1D::input_wrong(string coord){
...
int HexArray1D::size_of_string(const string &str){
	auto i=0;
...
void HexArray1D::setOldMove(const HexArray1D::Cell &c){			//Entering oldMove for undo operation.
...
void HexArray1D::print(){
	...
	for(int row=0; row<boardSize; row++){
		...
		for(int column=0; column<boardSize; column++){
			cout << hexCells[row*boardSize + column].getStatus() <<" ";
		}
		cout << endl;
	}
}

int HexArray1D::check_full(int x, int y, int visitedplaces[][26], char check, int &score){
	...
	*/
	if(check=='x' && x == boardSize-1 ){
		for(int m=0; m<boardSize; m++)
			if (visitedplaces[m][boardSize-1] == 0 && (hexCells[m*boardSize + boardSize-1].getStatus() == 'x' || hexCells[m*boardSize + boardSize-1].getStatus() == 'X') )
				{ makeLettersCapital('x', visitedplaces); score=100; return 2; }
	}
	
	if(check=='o' && y == boardSize-1 ){
		for(int m=0; m<boardSize; m++)
			if (visitedplaces[boardSize-1][m] == 0 && (hexCells[boardSize-1*boardSize + m].getStatus() == 'o' || hexCells[(boardSize-1)*boardSize + m].getStatus() == 'O') )
...
void HexArray1D::readFromFile(const string &filename){	//For reading the file.
	...
	for(i=0; i<boardSize; i++){
		...
		for(j=0; j<boardSize; j++){
			hexCells[i*boardSize + j].setStatus(inside[j]);
		}
	}
...
void HexArray1D::writeToFile(const string &filename){		//Wrtiting to file.
	/*
	...
	*/
	...
	for(int i=0; i< boardSize; i++){
		for(int j=0; j< boardSize; j++)
			file << hexCells[i*boardSize + j].getStatus();
		file << endl;
	}
...
int HexArray1D::getIsDone() const{
	return is_done2;
}

HexArray1D::Cell HexArray1D::operator()(int x,int y){	//I used this operator() in operator== to compare.
	bool flag=true;
	if(x<0 || y<0 || y>=boardSize || x>=boardSize) 
		flag = false;
	if(flag == false)	throw Errors("Error occured about the border.\n");
	return hexCells[y*boardSize + x];
}
